...
state_data <- aggregate(user_current_balance ~ user_address_state_abv, data = user_info, FUN=mean)
# rename the values to be consistent with the shape file state names
state_data <- rename(state_data, c("user_address_state_abv"="st", "user_current_balance"="average_balance"))

# merge on values to shape file
merged_info <- merge(x=states_info, y = state_data, by = "st", all = TRUE)

# create the choropleth (heat) map
png(filename="chorolpleth_avg_balance_by_state_bettRment.png", width=1440, height=950)

# plot the map!
qplot(
  long, lat, data = merged_info, group = group, 
  fill = average_balance, geom = "polygon"
)
